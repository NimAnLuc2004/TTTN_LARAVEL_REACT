...
class ProductDetailController extends Controller
{
    ...
    {
        $query = ProductDetail::join('products', 'products.id', '=', 'product_details.product_id')
            ->select("product_details.id", "product_details.product_id","product_details.name", "products.name as product_name", "product_details.color", "product_details.size", "product_details.price");
            if ($request->has('page')) {
                $prodetail = $query->paginate(10);
            } else {
                $prodetail = $query->get();
            }
        $result = [
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($prodetail->save()) {
            $this->writeLog('update', 'product_details', $prodetail->id, 'Cập nhật chi tiết sản phẩm'.$prodetail->toJson());
            return response()->json([
                'status' => true,
