...
class OrderController extends Controller
{
        ...
        return DB::transaction(function () use ($request) {
            try {
                ...
                foreach ($products as $item) {
                    ...
                    if ($variantQty == 0) {
                        $existingNotification = Notification::where('message', 'like', "%Biến thể sản phẩm {$product->name} (ID: {$product->id}, Biến thể: {$productDetail->name}, {$productDetail->color}, {$productDetail->size}) đã hết hàng%")
                            ->where('role', 'admin')
                            ->exists();
    ...
    public function confirmOrder(Request $request, $id)
    {
        try {
            ...
            if ($validated['payment_method'] === 'Paypal' || $validated['payment_method'] === 'VNPay') {
                $payment->status = 'pending'; // Status will be updated via callback
    ...
    protected function generatePaymentUrl($order, $payment)
    {
        ...
        if ($payment->payment_method === 'VNPay') {
            ...
            $vnp_ReturnUrl = env('VNPAY_RETURN_URL', 'http://localhost:3000/profile'); // Frontend URL with activeTab=orders
    ...
    public function vnpayCallback(Request $request)
    {
        ...
        try {
            ...
            $redirectUrl = env('FRONTEND_REDIRECT_URL', 'http://localhost:3000/profile');
