...
class ContactController extends Controller
{
    ...
    public function index()
    {
        $contacts = Contact::where('contacts.status', '!=', 0)
            ...
            ->select("contacts.id", "contacts.name", "contacts.replay_id", "contacts.user_id", "contacts.title", "contacts.status", "users.name as username", "contacts.phone", "contacts.email")
            ->paginate(10);
        $result = [
            'status' => true,
    ...
    public function replay($id)
    {
        ...
        $contact->replay_id = Auth::id(); // Gán ID người trả lời
        $contact->status = 3; // Trạng thái "Đã trả lời"
        $contact->updated_by = Auth::id();
        $contact->updated_at = date('Y-m-d H:i:s');

        if ($contact->save()) {
            $this->writeLog('replay', 'contacts', $contact->id, 'Trả lời liên hệ');
            $result = [
                'status' => true,
    ...
    public function status($id)
    {
        ...
        if ($contact->status !== 3) { // Chỉ thay đổi nếu không phải trạng thái "Đã trả lời"
            ...
            $result = [
                'status' => false,
                'message' => 'Không thể thay đổi trạng thái của liên hệ đã trả lời',
                'contact' => $contact
            ];
        }
        return response()->json($result);
