...
class ProductstoreController extends Controller
{
    ...
    public function store(StoreProductStoreRequest $request)
    {
        ...
        // Kiểm tra product_id trong ProductStore
        $productstore = ProductStore::where('product_id', $request->product_id)->first();

        if ($productstore) {
            // Cập nhật số lượng
            $newQty = $productstore->qty + $request->qty;
            // Kiểm tra nếu số lượng âm (tùy thuộc vào nghiệp vụ)
            if ($newQty < 0) {
                return response()->json([
                    'status' => false,
                    'message' => 'Số lượng trong kho không thể âm',
                    'productstore' => null
                ], 422);
            }
            ...
            $productstore->created_at = now(); // Sử dụng now() thay cho date()
