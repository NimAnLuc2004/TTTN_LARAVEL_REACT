...
const UserService = {
  ...
  getOrders: async () => {
    try {
      ...
      const config = token
        ? { headers: { Authorization: `Bearer ${token}` } }
        : {};
      const response = await httpAxios.get("frontend/orders", config);
      return response;
    } catch (error) {
      throw new Error(error.response?.message || "Không thể tải đơn hàng ");
    }
  },
  ...
  getVouchers: async () => {
    try {
      const response = await httpAxios.get("frontend/vouchers");
      return response;
    } catch (error) {
      throw new Error(
...
googleLogin: async (googleData) => {
    try {
        ...
        return {
            status: false,
            message: error.response?.data?.message || error.message || 'Đăng nhập Google thất bại'
        };
    }
},
};
