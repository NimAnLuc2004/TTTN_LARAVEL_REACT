...
const Dashboard = () => {
      ...
      try {
        ...
        if (result?.top_product) {
          const formattedData = result.top_product.map((item) => ({
            name: item.name,
            order_count: Number(item.total_quantity),
          }));
  ...
  // Tính giá trị tối đa của doanh thu và thêm khoảng đệm
  const maxRevenue = topRevenueProducts.length > 0
    ? Math.max(...topRevenueProducts.map(item => item.revenue))
    : 0;
  const yAxisMax = maxRevenue > 0 ? maxRevenue * 1.1 : 1000000; // Tăng 10% hoặc đặt mặc định 1M nếu không có dữ liệu
        ...
        <div className="bg-white p-5 rounded-xl shadow-xl text-black">
          ...
          >
            ...
            <YAxis
              ...
              domain={[0, yAxisMax]} // Sử dụng giá trị động cho trục Y
            />
