  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status) {
       ...
       if (response.require_logout) {
          // Đăng xuất nếu chỉnh sửa tài khoản đang đăng nhập
          await UserService.logout();
          localStorage.removeItem("user");
          localStorage.removeItem("token");
          toast.info("Thông tin tài khoản đã thay đổi. Vui lòng đăng nhập lại!");
          setTimeout(() => {
            navigate("/admin/login"); // Sử dụng navigate thay vì window.location
          }, 200);
        } else {
          navigate("/admin/user"); // Quay lại danh sách người dùng nếu không phải tài khoản đang đăng nhập
        }
      } else {
