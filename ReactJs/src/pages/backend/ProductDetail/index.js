...
const ProductDetailList = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (
          result &&
          result.prodetail.data &&
          result.prodetail.data.length > 0
        ) {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (
      window.confirm("Bạn có chắc chắn muốn xóa các chi tiết sản phẩm đã chọn?")
    ) {
      try {
        await Promise.all(
          selectedIds.map((id) => ProductDetailService.destroy(id))
        );
                    ...
                    <tr key={productdetail.id} className="border-b">
                      <td className="p-4 text-center">
                        <input
                          ...
                          onChange={() =>
                            handleCheckboxChange(productdetail.id)
                          }
                        />
                      </td>
