...
const MenuEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [brandRes, categoryRes, newRes, menuRes] = await Promise.all([
          BrandService.index(),
  ...
  const handleSubmit = async (e) => {
    ...
    menu.append("table_id", type === "custom" ? "" : tableid);
  ...
  const handleSelect = (type, item) => {
    ...
    switch (type) {
      case "brand":
        newLink = `https://zstore.click/product/all?brand_id=${item.id}`;
        break;
      case "category":
        newLink = `https://zstore.click/product/all?category_id=${item.id}`;
        break;
      case "new":
        newLink = `https://zstore.click/news/detail/${item.id}`;
        break;
      default:
        newLink = "";
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="min-w-full max-w-4xl bg-white shadow-md rounded-md p-8">
        <h1 className="text-3xl font-bold mb-6 text-center">Chỉnh sửa Menu</h1>
        ...
        <form className="grid grid-cols-2 gap-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <select
              className="shadow border rounded w-full py-2 px-3 text-gray-700"
              value={type === "brand" ? tableid : ""}
              onChange={(e) => {
                const selectedBrand =
                  brands.find((b) => b.id === parseInt(e.target.value)) || null;
          ...
          <div>
            ...
            <select
              className="shadow border rounded w-full py-2 px-3 text-gray-700"
              value={type === "new" ? tableid : ""}
              onChange={(e) => {
                const selectedNew =
                  news.find((n) => n.id === parseInt(e.target.value)) || null;
