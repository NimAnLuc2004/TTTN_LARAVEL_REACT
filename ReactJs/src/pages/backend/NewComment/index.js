...
const NewCommentList = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa newcomment này?")) {
      try {
        ...
        setNewComments(
          newcomments.filter((newcomment) => newcomment.id !== id)
        ); // Cập nhật state để loại bỏ newcomment đã xóa
  ...
  const handleDeleteSelected = async () => {
    ...
    if (window.confirm("Bạn có chắc chắn muốn xóa các bình luận đã chọn?")) {
      try {
        await Promise.all(
          selectedIds.map((id) => NewCommentService.destroy(id))
        );
        setNewComments((prev) =>
                ...
                return (
                  <tr key={newcomment.id} className="border-b">
                    ...
                    <td className="p-4">
                      ...
                      >
                        <IoEyeSharp className="inline" /> Xem bình luận khách
                        hàng
                      </Link>
                      <Link
