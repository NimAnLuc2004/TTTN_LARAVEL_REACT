...
const NotificationAdd = () => {
  ...
  const [selectedIds, setSelectedIds] = useState([]); // ID thông báo được chọn trong bảng
  const [selectedUserIds, setSelectedUserIds] = useState([]); // ID người dùng được chọn để gửi thông báo
  const [searchTerm, setSearchTerm] = useState("");
  const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" });
  const [selectAllUsers, setSelectAllUsers] = useState(false); // State for select all users
  // Pagination
  ...
  const handleUserSelect = (e) => {
    ...
    if (selectedValue && selectedValue !== "all") {
      ...
      setSelectedUserIds([parseInt(selectedValue)]);
  ...
  const handleAddNotification = async () => {
    if (!userId || !message || !role) {
      toast.error(
        "Vui lòng chọn người dùng, nhập nội dung thông báo và chọn quyền."
      );
      return;
    }
    ...
    try {
      if (userId === "all") {
        ...
        const notificationPromises = selectedUserIds.map((userId) => {
          ...
          return Notificationservice.insert(notificationData);
        });
        ...
        if (successfulNotifications.length > 0) {
          toast.success(
            `Thêm ${successfulNotifications.length} thông báo thành công!`
          );
          setNotification([
            ...notifications,
            ...successfulNotifications.map((res) => res.noti),
          ]);
          setMessage("");
          setUserId(null);
          setRole("user");
