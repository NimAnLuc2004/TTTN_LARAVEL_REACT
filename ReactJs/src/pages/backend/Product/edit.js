  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const categoryIds = result.product.categories
          ? result.product.categories.map((category) => category.id)
          : [];
        setCategoryId(categoryIds); // Gán mảng ID vào state
            ...
            <div className="flex gap-4 flex-wrap">
              {existingImages.length > 0 ? (
                existingImages.map((image, index) => (
                  <img
                    key={index}
                    src={urlImage + "product/" + image.image_url}
                    alt="Product"
