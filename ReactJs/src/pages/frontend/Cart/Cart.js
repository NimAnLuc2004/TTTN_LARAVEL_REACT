...
const Cart = () => {
  ...
  const handleCheckout = () => {
    ...
    const checkoutData = {
      ...
      selectedItems: selectedItems.map((item) => ({
        ...
        product_id: item.product_id,
        name: item.name,
        image: item.image.replace(urlImage + "product/", ""),
        price: item.price,
        quantity: item.quantity,
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6 min-h-screen flex items-center justify-center">
        <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg max-w-md w-full text-center">
          <p className="text-red-500 text-base sm:text-lg font-medium mb-4">
            Vui lòng đăng nhập để xem giỏ hàng
          </p>
          <a
            href="/login"
            className="inline-block bg-blue-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-blue-600 transition text-sm sm:text-base"
          >
    ...
    return (
      <div className="container mx-auto p-4 sm:p-6 min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center">
          <div className="w-10 h-10 sm:w-12 sm:h-12 border-4 border-orange-500 border-t-transparent rounded-full animate-spin" />
          <p className="text-gray-500 text-base sm:text-lg mt-4">Đang tải giỏ hàng...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="container mx-auto p-4 sm:p-6 min-h-screen flex items-center justify-center">
        <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg max-w-md w-full text-center">
          <p className="text-red-500 text-base sm:text-lg font-medium mb-4">{error}</p>
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm mb-4">
        <div className="hidden sm:grid sm:grid-cols-12 gap-4 text-sm font-semibold text-gray-600">
      ...
      {cartItems.length > 0 ? (
        cartItems.map(
          ({ id, name, price, quantity, image, selected, discountCode }) => {
            ...
            return (
              <div
                key={id}
                className="bg-white p-3 sm:p-4 rounded-xl shadow-sm mb-4 flex flex-col sm:grid sm:grid-cols-12 gap-3 sm:gap-4 text-sm"
              >
                <div className="flex items-center sm:col-span-4">
                  ...
                  <img
                    src={image}
                    alt={name}
                    className="w-14 h-14 sm:w-16 sm:h-16 object-cover rounded-md mr-3"
                  />
                  <div className="flex-1">
                    <p className="font-medium text-gray-800 truncate text-sm sm:text-base">{name}</p>
                    <div className="sm:hidden mt-2">
                      <span className="text-gray-700">Đơn giá: {price.toLocaleString()} ₫</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex sm:col-span-2 sm:justify-center">
                  <div className="flex items-center border border-gray-300 rounded-md w-24 sm:w-28 mx-auto sm:mx-0">
                    <button
                      onClick={() => handleQuantityChange(id, "decrease")}
                      className="px-2 sm:px-3 py-1 text-gray-600 hover:text-orange-500"
                    >
                      -
                    </button>
                    <span className="px-2 sm:px-3 py-1 text-gray-800">{quantity}</span>
                    <button
                ...
                <div className="sm:col-span-2">
                  <select
                    ...
                    className="w-full p-2 border border-gray-300 rounded-md focus:ring-orange-500 focus:border-orange-500 text-sm sm:text-base"
                  >
                ...
                <div className="sm:col-span-1 text-center text-orange-500 font-semibold">
                  {discountCode ? (
                    <div>
                      <del className="text-gray-500 text-xs sm:text-sm">
                        {originalTotalPrice.toLocaleString()} ₫
                      </del>
                      <div className="text-sm sm:text-base">{totalItemPrice.toLocaleString()} ₫</div>
                    </div>
                  ) : (
                    <div className="text-sm sm:text-base">{originalTotalPrice.toLocaleString()} ₫</div>
                  )}
                </div>
                <div className="sm:col-span-1 text-center">
                  <button
                    onClick={() => handleDeleteItem(id)}
                    className="text-red-500 hover:text-red-600 hover:underline text-sm sm:text-base"
                  >
                    Xóa
                  </button>
                </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm text-center">
          ...
          <a
            href="/"
            className="inline-block mt-4 text-orange-500 hover:text-orange-600 font-medium text-sm sm:text-base"
          >
            Tiếp tục mua sắm
          </a>
        </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm mt-4 sm:mt-6">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
            ...
            <div className="text-right">
              <span className="text-gray-600 text-sm sm:text-base">Tổng thanh toán: </span>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="flex-1 w-full">
              ...
              <select
                ...
                className="mt-1 w-full p-2 border border-gray-300 rounded-md focus:ring-orange-500 focus:border-orange-500 text-sm sm:text-base"
              >
            ...
            <button
              ...
              className="bg-orange-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-orange-600 transition text-sm sm:text-base w-full sm:w-auto"
            >
