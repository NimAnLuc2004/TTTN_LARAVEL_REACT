...
const urlImage = 'https://zstore.click/images/new/';
...
const News = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch('https://zstore.click/api/frontend/news/topics');
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        let url;
        if (!selectedTopicId) {
          url = `https://zstore.click/api/frontend/news/list/${limit}`;
        } else {
          url = `https://zstore.click/api/frontend/news/topic/${limit}/${selectedTopicId}`;
        }
        ...
        // Nếu là danh mục "Tất cả" và chưa có featured news, chọn tin đầu tiên làm featured (trừ mobile)
        if (!selectedTopicId && formattedNews.length > 0 && !isMobile && !featuredNews) {
  ...
  }, [selectedTopicId, limit, categories, isMobile, featuredNews]);
  ...
  return (
    <div className="container mx-auto p-5 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white p-8 rounded-lg shadow-sm">
        ...
        {/* Featured news (chỉ hiển thị khi chọn "Tất cả" và không phải mobile) */}
        {!loading && featuredNews && !selectedTopicId && !isMobile && (
          <div className="mb-8 featured-news">
