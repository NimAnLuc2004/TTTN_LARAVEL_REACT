...
const NewsDetail = () => {
    ...
    const fetchNewsDetail = async () => {
      try {
        ...
        const response = await fetch(
          `https://zstore.click/api/frontend/news/detail/${id}`
        );
        ...
        const commentsResponse = await fetch(
          `https://zstore.click/api/frontend/news/comments/${id}`
        );
        ...
        const relatedResponse = await fetch(
          `https://zstore.click/api/frontend/news/topic/3/${data.news.topic.id}`
        );
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://zstore.click/api/frontend/news/comment/${id}`,
        {
          method: "POST",
          headers: {
  ...
  const handleReaction = async (commentId, reaction) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://zstore.click/api/frontend/news/comment/${commentId}/reaction`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl">
      ...
      <motion.div variants={detailVariants} initial="hidden" animate="visible">
        ...
        {images.length > 0 && (
          <div className="mb-6 rounded-lg overflow-hidden shadow-md carousel-container">
            <Carousel
            ...
            >
              {images.map((img, index) => (
                  ...
                  <img
                    src={`https://zstore.click/images/new/${img}`}
                ...
                <Link to={`/news/detail/${related.id}`} key={related.id}>
                  ...
                  >
                    {related.images.length > 0 ? (
                      <img
                        src={`https://zstore.click/images/new/${related.images[0]}`}
                        alt={related.title}
