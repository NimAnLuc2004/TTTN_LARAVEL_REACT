...
const CustomerOrder = () => {
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    if (!result.isConfirmed) {
      return; // Nếu người dùng nhấn "Không", dừng lại
    }
  ...
  return (
    <div className="container mx-auto p-5 bg-gray-100 min-h-screen">
      <div className="bg-white p-8 rounded-lg shadow-sm">
        ...
        <Link
          to="/track-order"
          className="flex items-center gap-2 text-blue-600 hover:text-blue-800 font-medium text-lg transition-colors duration-300"
                        ...
                        <div className="border-t border-gray-200 pt-3 mb-3">
                          ...
                          {(order.items || []).map((item, idx) => (
                            <div key={idx} className="flex gap-3 mb-3">
                              <img
                                src={item.image || "/placeholder-image.png"} // Thêm ảnh mặc định nếu không có ảnh
