...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !email || !password || images.length === 0 || !address || !phone) {
      setError("Vui lòng nhập đầy đủ thông tin và tải lên ít nhất một ảnh");
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 overflow-hidden">
      ...
      {/* Register Card */}
      <div className="relative z-10 bg-white bg-opacity-10 backdrop-blur-lg p-8 rounded-2xl shadow-2xl w-full max-w-md border border-white border-opacity-20">
        <h2 className="text-4xl font-extrabold text-center text-white mb-8 animate-fade-in">
          Đăng Ký
        </h2>
        ...
        {message && (
          <p className="text-green-300 text-sm mb-4 text-center animate-fade-in">
            {message}
          </p>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data">
          ...
          <div className="relative">
            <input
              ...
              disabled={isLoading}
            />
            <label className="block text-white text-opacity-70 text-sm mt-2">
              Ảnh Đại Diện (Tải lên ít nhất một ảnh)
            </label>
          </div>
        ...
        {/* Signin Link */}
        <p className="text-center text-white text-opacity-80 mt-6">
          Đã có tài khoản?{" "}
          <a
            href="/signin"
