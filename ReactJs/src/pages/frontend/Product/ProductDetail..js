...
const ProductDetail = () => {
  ...
  const handleQuantityChange = (action) => {
    ...
    if (action === "increase" && quantity < selectedVariant.stock_qty) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        {
          user_id: userId,
          product_id: selectedVariant.id, // Use variant ID
          quantity,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      if (!cartResponse.data.status) {
        setError(cartResponse.data.message || "Không thể thêm vào giỏ hàng.");
        return;
      }
        ...
        selectedItems: [
          {
            ...
            product_id: selectedVariant.id, // Use variant ID for order processing
