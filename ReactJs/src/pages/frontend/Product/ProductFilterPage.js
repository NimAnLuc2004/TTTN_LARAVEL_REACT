...
const ProductFilterPage = () => {
  ...
  const calculateSalePrice = (minPrice, discountPercent) => {
    ...
    if (!discountPercent) {
      return { salePrice: formatVND(price), comparePrice: null };
    }
    const salePrice = price * (1 - discountPercent / 100);
    return { salePrice: formatVND(salePrice), comparePrice: formatVND(price) };
  };
          ...
          fetchedProducts = [...fetchedProducts].sort((a, b) => {
            if (filters.sortPrice !== "default") {
              const priceA = parseFloat(
                (a.min_price || a.price) * (1 - (a.discount_percent || 0) / 100)
              );
              const priceB = parseFloat(
                (b.min_price || b.price) * (1 - (b.discount_percent || 0) / 100)
              );
              if (priceA !== priceB) {
